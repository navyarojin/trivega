...
flowchart LR
    ...
    subgraph TB["🔧  Testbench  (tb_vega.sv)"]
        direction TB
        INIT["Load .mem → DDR\n$readmemh + preload_word"]
        CFG["Program CSR via\nAXI4-Full BFM\naddresses · mode · start"]
        POLL["Poll done / error\n@ reg 0x08"]
        CHK["Validate result\ncheck best_idx range"]
        INIT --> CFG --> POLL --> CHK
    end
    ...
    subgraph DUT["⚡  DUT  (vega_core.sv)"]
        direction TB
        CSR["AXI4-Full CSR"]
        DMA["DMA engine\nimage mode · matmul mode"]
        PIPE["Scoring pipeline\nMAC → cosine → affordance\n→ fuse → top-1"]
        CSR --> DMA --> PIPE
    end
    ...
    %% ── Styling ──────────────────────────────────────────────────────
    classDef sw   fill:#0f4c4c,stroke:#2dd4bf,color:#ccfbf1
    classDef tb   fill:#1a3a1a,stroke:#4ade80,color:#dcfce7
    classDef dut  fill:#2d1b4e,stroke:#a78bfa,color:#ede9fe
    classDef ddr  fill:#3b2200,stroke:#fb923c,color:#ffedd5
    classDef out  fill:#1e3a5f,stroke:#60a5fa,color:#dbeafe
    classDef ref  fill:#3b1f1f,stroke:#f87171,color:#fee2e2

    class PY,MEM sw
    class INIT,CFG,POLL,CHK tb
    class CSR,DMA,PIPE dut
    class DDR ddr
    class R1,R2,R3 out
    class REF ref
```
